...
class DataFirehoseProxyStack(Stack):
  ...
  def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
    ...
    apigw_datafirehose_role = aws_iam.Role(self, "APIGatewayRoleToAccessDataFirehose",
      role_name=f"APIGatewayRoleToAccessDataFirehose-{datetime.utcnow().strftime('%Y%m%d')}",
      assumed_by=aws_iam.ServicePrincipal('apigateway.amazonaws.com'),
      inline_policies={
        'DataFirehoseWriteAccess': apigw_kds_access_role_policy_doc
      },
      managed_policies=[
        aws_iam.ManagedPolicy.from_aws_managed_policy_name('AmazonKinesisFirehoseReadOnlyAccess')
      ]
    )
    ...
    datafirehose_proxy_api = aws_apigateway.RestApi(self, "DataFirehoseProxyAPI",
      rest_api_name="datafirehose-proxy",
      description="An Amazon API Gateway REST API that integrated with an Amazon Data Firehose.",
      ...
      endpoint_export_name="DataFirehoseProxyAPIEndpoint-{}".format(''.join(random.sample(string.ascii_letters, k=3)))
    )
    ...
    streams_resource = datafirehose_proxy_api.root.add_resource("streams")

    list_streams_options = aws_apigateway.IntegrationOptions(
      credentials_role=apigw_datafirehose_role,
...
'''
    ...
    cdk.CfnOutput(self, 'RestApiEndpointUrl',
      value=datafirehose_proxy_api.url,

...
class MwaaStack(cdk.Stack):
  ...
  def __init__(self, scope: cdk.Construct, construct_id: str, **kwargs) -> None:
    ...
    MY_MWAA_ENV_NAME = "my-airflow-dev"
    ...
    sg_mwaa = aws_ec2.SecurityGroup(self, "AirflowSG",
      ...
      security_group_name='airflow-security-group-{}'.format(MY_MWAA_ENV_NAME)
    )
    ...
    mwaa_execution_policy_doc.add_statements(aws_iam.PolicyStatement(**{
      "effect": aws_iam.Effect.ALLOW,
      # arn:{partition}:{service}:{region}:{account}:{resource}{sep}{resource-name}
      "resources": [self.format_arn(service="sqs", resource="*",
        resource_name="airflow-celery-*", sep=":")],

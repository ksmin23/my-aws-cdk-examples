...
class ECRStack(Stack):
  ...
  def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
    ...
    self.repository.add_lifecycle_rule(max_image_age=cdk.Duration.days(7),
      rule_priority=1,
      tag_status=aws_ecr.TagStatus.UNTAGGED
    )

    # keep last 3 images
    self.repository.add_lifecycle_rule(max_image_count=3,
      rule_priority=2,
      tag_status=aws_ecr.TagStatus.ANY
    )

    src_docker_image_version = self.node.try_get_context('src_docker_image_version')
    image_version = self.node.try_get_context('image_version') or "latest"
